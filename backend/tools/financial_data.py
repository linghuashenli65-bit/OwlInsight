...
def _pivot_hk_report(df: pd.DataFrame) -> pd.DataFrame:
    """将港股 API 返回的标准化格式（每行一个指标）透视成宽表（每行一个报告期）.

    HK 返回列: [SECUCODE, SECURITY_CODE, SECURITY_NAME_ABBR, REPORT_DATE, STD_ITEM_NAME, AMOUNT, ...]
    透视后: index=REPORT_DATE, columns=STD_ITEM_NAME, values=AMOUNT
    """
    ...
    pivoted.index.name = "REPORT_DATE"
    pivoted.reset_index(inplace=True)
    # 取公司名（所有行都相同）
    company_name = ""
    if "SECURITY_NAME_ABBR" in df.columns:
        company_name = str(df["SECURITY_NAME_ABBR"].iloc[0]) if not df.empty else ""
    pivoted.attrs["company_name"] = company_name
    return pivoted
...
def get_financials(symbol: str, period: str = "annual") -> list[dict[str, Any]]:
    ...
    A 股使用 ak.stock_yjbb_em（业绩报表），港股使用 stock_financial_hk_report_em。
    period 参数仅用于控制备选接口切换。
    """
    # ── 港股分支 ──
    if _is_hk_code(symbol):
        try:
            df = ak.stock_financial_hk_report_em(stock=symbol, symbol="利润表", indicator="年报")
        except Exception as e:
            logger.warning("获取港股利润表失败 (%s): %s", symbol, e)
            return []
        if df is None or df.empty:
            logger.warning("港股利润表为空 (%s)", symbol)
            return []
        pivoted = _pivot_hk_report(df)
        if pivoted.empty:
            return []
        company_name = pivoted.attrs.get("company_name", "")
        results = []
        for _, row in pivoted.iterrows():
            report_date = str(row.get("REPORT_DATE", ""))
            try:
                rd = datetime.strptime(report_date, "%Y-%m-%d %H:%M:%S").date()
            except ValueError:
                try:
            ...
            results.append({
                ...
                "revenue": _safe_float(row.get("营业额")),
                "net_profit": _safe_float(row.get("股东应占溢利")),
                "gross_profit": _safe_float(row.get("毛利")),
                "operating_profit": _safe_float(row.get("经营溢利")),
    ...
    # 精确匹配：先转字符串，去掉 .SH/.SZ 后缀，再比较
    df_filtered = df[df[col_code].astype(str).str.replace(r"\.(SH|SZ|sh|sz)$", "", regex=True).str.strip() == symbol].copy()
    if df_filtered.empty:
        logger.warning("未找到股票 %s 的利润表数据（A 股财报接口无此代码）", symbol)
        return []
...
def _hk_calc_bs_asset(row: pd.Series) -> Optional[float]:
    """从香港资产负债表行中计算总资产（流动资产合计 + 非流动资产合计）. """
    cur = _safe_float(row.get("流动资产合计"))
    non_cur = _safe_float(row.get("非流动资产合计"))
    if cur is not None and non_cur is not None:
        return cur + non_cur
    return cur or non_cur or None

def _hk_calc_bs_liability(row: pd.Series) -> Optional[float]:
    """从香港资产负债表行中计算总负债（流动负债合计 + 非流动负债合计）. """
    cur = _safe_float(row.get("流动负债合计"))
    non_cur = _safe_float(row.get("非流动负债合计"))
    if cur is not None and non_cur is not None:
        return cur + non_cur
    return cur or non_cur or None
...
def get_balance_sheet(symbol: str, period: str = "annual") -> list[dict[str, Any]]:
    ...
    A 股使用 stock_balance_sheet_by_report_em，港股使用 stock_financial_hk_report_em。
    """
    # ── 港股分支 ──
    if _is_hk_code(symbol):
        try:
            df = ak.stock_financial_hk_report_em(stock=symbol, symbol="资产负债表", indicator="年报")
        except Exception as e:
            logger.warning("获取港股资产负债表失败 (%s): %s", symbol, e)
            return []
        if df is None or df.empty:
            return []
        pivoted = _pivot_hk_report(df)
        ...
        for _, row in pivoted.iterrows():
            ...
            results.append({
                ...
                "report_period": report_date,
                "period_rank": _period_rank_from_date(rd),
                "total_assets": _hk_calc_bs_asset(row),
                "total_liabilities": _hk_calc_bs_liability(row),
                "equity": _safe_float(row.get("股东权益")),
                "cash_and_equivalents": _safe_float(row.get("现金及等价物")),
                "accounts_receivable": _safe_float(row.get("应收账款")) or _safe_float(row.get("预付款按金及其他应收款")),
                "raw_data": row.to_dict(),
            })
        return results
...
def get_cash_flow(symbol: str, period: str = "annual") -> list[dict[str, Any]]:
    ...
    A 股使用 stock_cash_flow_sheet_by_report_em，港股暂不支持。
    """
...
def get_stock_price_history(
    ...
    # ── 港股接口（腾讯行情接口，akshare 的 stock_hk_hist 被服务器阻断） ──
    if is_hk:
        max_retries = 3
        last_err = None
        for attempt in range(max_retries):
            try:
                ...
                if df is not None and not df.empty:
                    if attempt > 0:
                        logger.info("港股股价重试成功 (%s), 第 %d 次", symbol, attempt + 1)
                    results = []
                    for _, row in df.iterrows():
...
def get_company_name(symbol: str) -> str:
    """根据股票代码获取公司简称."""
    # 港股：从 HK 利润表接口获取（列名为 SECURITY_NAME_ABBR）
    if _is_hk_code(symbol):
        try:
            df = ak.stock_financial_hk_report_em(stock=symbol, symbol="利润表", indicator="年报")
            if df is not None and not df.empty and "SECURITY_NAME_ABBR" in df.columns:
                name = str(df["SECURITY_NAME_ABBR"].iloc[0])
                if name:
                    return name
        except Exception:
            pass
